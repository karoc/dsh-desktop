...
| 外网 / registry 访问 | **一律走 npm / curl 的传输，不要裸 `fetch`**：Node 的 fetch 不读 npm 的 `.npmrc` 代理配置，且代理变量与 `NODE_USE_ENV_PROXY` 在**进程启动时**采样（脚本内再设无效）。2026-09-22 一天内因此踩到三处：`audit-preinstalled` 在零数据上打印通过、`release-body` 把 dsh 版本写成裸 `unknown`、`fetch-node` 全新构建下不到 Node（一直没暴露是因为二进制已缓存）。判据 grep：`grep -rn "fetch(" scripts/` |
