...
> ⚠️ **已被取代（2026-09-21）**：dsh 0.1.6-alpha.2 自带插件管理（`@deepseek-ai/dsh-plugin-manager`
> + Web 侧边栏 Plugins 页），本计划交付的壳内插件管理（控制台窗口 / 页内面板 / `/plugins/*`
> 桥端点 / 预装插件的更新与恢复出厂）已**整体移除**。取代方案见
> [`2026-09-21-dsh-0.1.6-alpha.2-upgrade-plan.md`](2026-09-21-dsh-0.1.6-alpha.2-upgrade-plan.md)。
> 本文保留为历史记录（当时的取舍与实现细节）。
>
> 状态：**已废弃**（历史文档）
