...
//     missing — a killed npm reify, exactly the 0.3.1 bug);
//   * a fake registry serves a REAL minimal dsh tarball (lib/bin.js included);
//   * the manager must detect "installed but broken", remove it, reinstall
//     from the fake registry, and successfully LAUNCH dsh.
...
// ── build a real minimal @deepseek-ai/dsh tarball with lib/bin.js ───────────
const pkgDir = join(tmp, 'pkg')
mkdirSync(join(pkgDir, 'package', 'lib'), { recursive: true })
writeFileSync(join(pkgDir, 'package', 'package.json'), JSON.stringify({
  name: '@deepseek-ai/dsh',
  version: '9.9.9-new',
  type: 'module',
  bin: { dsh: 'lib/bin.js' },
}, null, 2))
writeFileSync(join(pkgDir, 'package', 'lib', 'bin.js'), `console.log('dsh web: http://127.0.0.1:12345')\nsetInterval(() => {}, 1000)\n`)
const tarballPath = join(tmp, 'dsh.tgz')
execFileSync('tar', ['-czf', tarballPath, '-C', pkgDir, 'package'])
const tarball = readFileSync(tarballPath)

// ── fake registry: valid packument + real tarball ───────────────────────────
...
// Wait long enough for: gate → remove → npm install → launch → dsh "running".
...
if (pass) console.log('\nPASS: broken install auto-repairs and dsh launches')
else console.error('\n--- manager output (tail) ---\n' + output.slice(-2500))
