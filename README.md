...
- **壳内代理**：所有 dsh 出站流量（模型请求、web 搜索、npm 安装/更新、插件、子代理）都经壳内置的本地正向代理；默认直连，可在设置里勾选哪些主机走上游代理（含可选账号密码），保存即生效。**入口 = dsh 页面右下角「插件」控制台面板里的「代理设置」+ 托盘「代理设置…」**，打开独立设置窗口，不打断 dsh 页面。
...
src/                      Tauri 前端加载页（纯静态，无打包器）
  ...
  settings.html / settings.js  独立代理设置窗口（控制台面板/托盘打开）
src-tauri/                Tauri 2 壳：
...
- **勾选「删除应用程序数据」= 清空全部本地数据**，下次启动会重新安装 dsh（0.3.2 起不会再卡死，
